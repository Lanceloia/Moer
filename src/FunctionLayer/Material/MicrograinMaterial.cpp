...
std::shared_ptr<BxDF> MicrograinMaterial::getBxDF(const Intersection &intersect) const {
...
std::shared_ptr<BSSRDF> MicrograinMaterial::getBSSRDF(const Intersection &intersect) const {
    return std::shared_ptr<BSSRDF>();
}

MicrograinMaterial::MicrograinMaterial(const Json &json) {
